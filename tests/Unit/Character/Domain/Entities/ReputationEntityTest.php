...
test('reputation', function (int $points, string $operation, int $expectedPoints, string $expectedBadge): void {
    $reputation = new ReputationEntity($points);
    $reputation->handleReputation($operation);

    expect($reputation->getBadge())->toEqual($expectedBadge);
    expect($reputation->getPoints())->toEqual($expectedPoints);
})->with('reputationDataProvider');
