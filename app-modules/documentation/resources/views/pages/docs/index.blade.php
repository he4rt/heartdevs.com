...
$page ??= 'index';
$markdownPath = base_path(sprintf('docs/%s.md', $page));
...
$versions = DocumentationHelper::getAvailableVersions();
            ...
            <div class="lg:flex">
                ...
                >
                    <nav class="space-y-8 p-6">
                        @if (count($versions) > 0)
                            <div>
                                <h3 class="mb-3 text-sm font-semibold text-zinc-900 dark:text-white">Versões</h3>
                                <ul class="space-y-2">
                                    @foreach ($versions as $ver)
                                        <li>
                                            <a
                                                href="/docs/{{ $ver['slug'] }}"
                                                class="flex items-center gap-2 text-sm text-zinc-600 hover:text-zinc-900 dark:text-zinc-400 dark:hover:text-white"
                                            >
                        ...
                        <div>
                            <h3 class="mb-3 text-sm font-semibold text-zinc-900 dark:text-white">Documentação</h3>
                            <ul class="space-y-2">
                                <li>
                                    <a
                                        href="/docs"
                                        class="{{ $page === 'index' ? 'font-semibold text-zinc-900 dark:text-white' : 'text-zinc-600 hover:text-zinc-900 dark:text-zinc-400 dark:hover:text-white' }} flex items-center gap-2 text-sm"
                                    >
