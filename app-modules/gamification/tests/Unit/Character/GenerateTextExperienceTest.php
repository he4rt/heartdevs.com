...
test('empty message returns minimum 1 xp', function (): void {
    expect(Character::generateTextExperience('', 1, isSupporter: false))->toBe(1);
});

test('short messages at level 1 return minimum 1 xp', function (string $message): void {
    expect(Character::generateTextExperience($message, 1, isSupporter: false))->toBe(1);
})->with([
    'hello' => ['hello'],
...
test('xp scales with message length', function (int $length, int $level, int $expectedXp): void {
    ...
    expect(Character::generateTextExperience($message, $level, isSupporter: false))->toBe($expectedXp);
})->with([
...
test('supporter doubles xp', function (): void {
    ...
    $regularXp = Character::generateTextExperience($message, 10, isSupporter: false);
    $supporterXp = Character::generateTextExperience($message, 10, isSupporter: true);

    expect($supporterXp)->toBe($regularXp * 2);
});

test('supporter gets minimum 2 xp for short messages', function (): void {
    expect(Character::generateTextExperience('hi', 1, isSupporter: true))->toBe(2);
});
...
test('multibyte characters count by character not byte', function (): void {
    ...
    expect(Character::generateTextExperience($mbMessage, 1, isSupporter: false))
        ->toBe(Character::generateTextExperience($asciiMessage, 1, isSupporter: false));
});
