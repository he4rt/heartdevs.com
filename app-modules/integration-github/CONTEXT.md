...
| **Allowlist**    | The set of repos that count, stored in `github_repositories` and managed in the admin panel. Ingestion (backfill + webhook) is scoped to it.      | All org repos (the org webhook delivers everything; we filter)      |
