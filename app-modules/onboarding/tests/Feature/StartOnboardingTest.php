...
test('starting Welcome persists the onboarding and its initial form step', function (): void {
    ...
    $onboarding = resolve(StartOnboarding::class)->handle($tenant, $user, OnboardingType::Welcome);
    ...
    expect($onboarding->exists)->toBeTrue()
        ...
        ->and($onboarding->tenant_id)->toBe($tenant->id)
...
test('starting Welcome twice is idempotent and never duplicates rows', function (): void {
    ...
    $first = resolve(StartOnboarding::class)->handle($tenant, $user, OnboardingType::Welcome);
    $second = resolve(StartOnboarding::class)->handle($tenant, $user, OnboardingType::Welcome);
