...
test('it counts distinct web and discord actives within the current window', function (): void {
    ...
    Timeline::factory()->create(['user_id' => $userA->id, 'created_at' => Date::create(2_026, 6, 10, 12, 0, 0, $tz)]);
    Timeline::factory()->create(['user_id' => $userA->id, 'created_at' => Date::create(2_026, 6, 12, 12, 0, 0, $tz)]);
    Timeline::factory()->create(['user_id' => $userB->id, 'created_at' => Date::create(2_026, 6, 1, 12, 0, 0, $tz)]);
    Timeline::factory()->create(['user_id' => $userC->id, 'created_at' => Date::create(2_026, 1, 1, 12, 0, 0, $tz)]);

    // Discord: one distinct identity with a message inside the window.
    $identity = ExternalIdentity::factory()->create();
    Message::factory()->create(['external_identity_id' => $identity->id, 'sent_at' => Date::create(2_026, 6, 11, 12, 0, 0, $tz)]);
