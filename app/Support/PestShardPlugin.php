...
/**
 * Drop-in replacement for Pest's built-in `--shard` plugin that works with a
 * modular monorepo (internachi/modular, tests under `app-modules/../tests`).
 *
 * WHY THIS EXISTS
 * ---------------
 * Pest's `Pest\Plugins\Shard` discovers the shardable test classes from
 * `--list-tests` with a regex hard-coded to the root `Tests\` namespace:
 *
 *     preg_match_all('/ - (?:P\\)?(Tests\\[^:]+)::/', $output, $matches);
 *
 * Our module tests are namespaced `Appmodules\<module>\tests\...` (lowercase
...
final class PestShardPlugin implements AddsOutput, HandlesArguments, Terminable
{
    ...
    private function allTests(array $arguments): array
    {
        ...
        // namespaced `Appmodules\<module>\tests\...`. Match ANY class instead.
